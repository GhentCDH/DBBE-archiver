...
def run_test():
    ...
    # SQLite
    sqlite_cursor.execute("SELECT id, name FROM metre")
    sqlite_set = {
        (row[0], normalize_string(row[1]))
        for row in sqlite_cursor.fetchall()
    }

    pg_cursor.execute('SELECT idmeter, name FROM data.meter')
    pg_set = {
        (row[0], normalize_string(row[1]))
    ...
    return compare_sets("metre", sqlite_set, pg_set, label="ID + Name")