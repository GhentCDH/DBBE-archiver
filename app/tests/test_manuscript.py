...
def run_test():
    ...
    sqlite_cursor.execute("SELECT id FROM manuscript")
    sqlite_set = {normalize_id(row[0]) for row in sqlite_cursor.fetchall()}

    pg_cursor.execute('SELECT "identity" FROM data.manuscript')
    pg_set = {normalize_id(row[0]) for row in pg_cursor.fetchall()}

    sqlite_conn.close()
    pg_conn.close()

    return compare_sets("manuscripts", sqlite_set, pg_set, label="Manuscript ID")