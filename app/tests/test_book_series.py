...
def run_test():
    ...
    sqlite_cursor.execute("SELECT id FROM book_series")
    sqlite_set = {normalize_id(row[0]) for row in sqlite_cursor.fetchall()}

    pg_cursor.execute('SELECT "identity" FROM data.book_series')
    pg_set = {normalize_id(row[0]) for row in pg_cursor.fetchall()}

    sqlite_conn.close()
    pg_conn.close()

    return compare_sets("book_series", sqlite_set, pg_set, label="Series ID")